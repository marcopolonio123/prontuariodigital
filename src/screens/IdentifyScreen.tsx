...
import {
  IconAlert, IconBell, IconCamera, IconCheck, IconChevronRight, IconFace, IconFingerprint,
  IconMapPin, IconPlus, IconRefresh, IconSearch, IconSpinner, IconUpload, IconUsers,
...
export function IdentifyScreen({
  ...
  const runPhotoPipeline = async (url: string) => {
    ...
    try {
      ...
      emitLog({
        method: 'face',
        patientId: status !== 'none' && best ? best.patient.id : null,
        patientName: status !== 'none' && best ? best.patient.name : '—',
        confidence,
        quality: null,
        result: status,
        thumb,
    ...
    onPatientsUpdated(
      patients.map((p) =>
        p.id === activePatient.id
          ? { ...p, missing: { ...p.missing, active: false, history: [...p.missing.history, { id: uid(), at: Date.now(), kind: 'found', text: foundText.trim() || `Localizada após identificação no app (por ${accountName}).` }] } }
          : p,
      ),
    );
    emitLog({
  ...
  if (patients.filter((p) => !p.archived).length === 0) {
    return (
      <EmptyState
    ...
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rise space-y-4" style={{ animationDelay: '90ms' }}>
            ...
            >
              {method === 'face' ? (
                ...
                <div className="flex h-full flex-col justify-center gap-3 p-5 sm:p-7">
                  <FingerprintPad onComplete={(t, q) => void onFingerComplete(t, q)} hint="Pressione e segure o sensor para ler a digital" />
                </div>
              )}
            </div>
          ...
          {result && (
            <div className="flex items-center justify-center rounded-xl border border-line bg-card p-4 shadow-lift">
              <Gauge
                value={result.confidence}
                label={result.status === 'match' ? 'identidade confirmada' : result.status === 'review' ? 'revisão manual' : 'sem correspondência'}
                tone={result.status === 'match' ? 'moss' : result.status === 'review' ? 'warn' : 'mute'}
              />
            </div>
          )}
          ...
          <p className="flex items-start gap-2 text-[12px] leading-relaxed text-mute">
            ...
            <span>
              ...
              fotos). A leitura de digital é simulada — navegadores não expõem sensores biométricos — e usa um template
              local.
            </span>
          </p>
        </section>
        ...
        <aside className="rise space-y-4" style={{ animationDelay: '130ms' }}>
          <div className="rounded-xl border border-line bg-card p-4 shadow-lift">
            <h2 className="mb-3 font-display text-sm font-bold uppercase tracking-[0.14em] text-mute">Candidatos da base</h2>
            {!result && !scanning && (
                    ...
                    <li key={c.patient.id}>
                      <button
                        ...
                        className={`w-full rounded-lg border p-2.5 text-left transition-all active:scale-[0.98] ${
                          picked ? 'border-moss-500 bg-moss-50 shadow-sm' : isBest ? 'border-moss-500/40 bg-card hover:border-moss-300' : 'border-line bg-card hover:border-pine-200'
                        }`}
                      >
                ...
                {log.slice(0, 6).map((e) => (
                  <li key={e.id} className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-[11px] text-mute/80">por {e.byName || 'conta local'}</p>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {activePatient.missing.active && (
            <div className="overflow-hidden rounded-xl border-2 border-danger-500 bg-danger-100/70 shadow-lift">
              <div className="flex flex-wrap items-center gap-3 bg-danger-500 px-4 py-2.5 text-white">
                ...
                <p className="font-display text-sm font-bold uppercase tracking-[0.18em]">Pessoa desaparecida — alerta ativo</p>
                <span className="ml-auto font-mono text-xs opacity-90">há {daysSince(activePatient.missing.since)} dia(s)</span>
              </div>
      ...
      <CameraCapture open={camOpen} onClose={() => setCamOpen(false)} onCapture={(url) => void runPhotoPipeline(url)} title="Identificação por retrato" />
