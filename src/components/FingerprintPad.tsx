...
export function FingerprintPad({
  ...
  const start = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      /* alguns ambientes não suportam capture */
    }
    samplesRef.current = [];
  ...
  const onKey = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if ((e.key === 'Enter' || e.key === ' ') && !scanning && !done) {
      ...
      setScanning(true);
      setMessage('Mantenha pressionado…');
      timerRef.current = window.setInterval(tick, 50);
      window.setTimeout(() => {
        if (timerRef.current !== null) finish();
      }, 2600);
    }
  };
    ...
    <div>
      ...
      >
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          {[190, 150, 110].map((d) => (
            <span key={d} className="absolute rounded-full border border-pine-700/70" style={{ width: d, height: d }} />
          ))}
        </div>
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-line">
        <div
          className={`h-full rounded-full transition-[width] duration-150 ease-linear ${done ? 'bg-moss-500' : 'stripes bg-moss-600'}`}
