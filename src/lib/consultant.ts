...
export interface MedOption {
  name: string;
  contra: string[]; // tags que contraindicam
  caution: string[]; // tags que exigem cautela
  note: string;
}
...
export const KB: KbEntry[] = [
  {
    id: 'febre',
    keys: ['febre', 'febril', 'temperatura alta', 'calafrio'],
    label: 'Febre',
    ...
    sources: ['Bula — Paracetamol/Dipirona (ANVISA)', 'Caderno de Atenção Básica — Febre (Ministério da Saúde)', 'MedlinePlus — Fever (NIH)'],
  },
  {
    id: 'cefaleia',
    keys: ['dor de cabeca', 'dor na cabeca', 'cefaleia', 'enxaqueca', 'cabeca doendo'],
    label: 'Dor de cabeça',
    ...
    options: [
      ...
      { name: 'Ibuprofeno 400 mg', contra: ['nsaid', 'ulcera', 'anticoagulante', 'renal'], caution: ['cardiopatia', 'hipertensao', 'idoso'], note: 'Útil em enxaqueca leve; evitar uso frequente (risco de cefaleia rebote).' },
    ],
    redFlags: ['A pior dor de cabeça da vida, súbita', 'Febre + rigidez de nuca', 'Alteração de fala, força ou visão — emergência'],
    sources: ['Protocolo Clínico — Cefaleias (Ministério da Saúde)', 'Bula — analgésicos (ANVISA)', 'IHS — classificação de cefaleias'],
  },
  {
    id: 'estomago',
    keys: ['estomago', 'azia', 'queimacao', 'gastrite', 'enjoo', 'nausea', 'digestao'],
    label: 'Desconforto gástrico / azia',
    ...
    options: [
      ...
      { name: 'Omeprazol 20 mg', contra: [], caution: ['idoso'], note: 'Uso em crises recorrentes, antes do café da manhã, por curto período.' },
    ...
    sources: ['Bula — Omeprazol (ANVISA)', 'Caderno de Atenção Básica — Problemas digestivos (MS)', 'ACG — diretrizes de refluxo'],
  },
  {
    id: 'alergia-pele',
    keys: ['coceira', 'urticaria', 'alergia na pele', 'manchas vermelhas', 'prurido', 'brotoeja'],
    label: 'Reação alérgica de pele',
    ...
    redFlags: ['Inchaço de lábios, língua ou rosto', 'Falta de ar ou chiado — usar caneta de epinefrina se prescrita e chamar SAMU 192', 'Lesões com pus ou febre associada'],
    sources: ['Bula — Loratadina (ANVISA)', 'ASBAI — orientações de urticária', 'Manual de Alergia e Imunologia (SBPT)'],
  },
  {
    id: 'gripe',
    keys: ['gripe', 'resfriado', 'tosse', 'coriza', 'nariz entupido', 'dor de garganta', 'congestao'],
    label: 'Gripe / resfriado',
    ...
    options: [
      ...
      { name: 'Soro fisiológico nasal + hidratação', contra: [], caution: [], note: 'Medida segura para todas as idades; a base do tratamento.' },
    ],
    redFlags: ['Falta de ar ou dor no peito', 'Febre alta por mais de 3 dias', 'Idosos, gestantes e bebês: avaliação médica precoce'],
    sources: ['Caderno de Atenção Básica — Gripes e resfriados (MS)', 'Bula — Loratadina (ANVISA)', 'CDC — cuidados com resfriados'],
  },
  {
    id: 'muscular',
    keys: ['dor muscular', 'dor no corpo', 'inflamacao', 'torcao', 'lesao', 'lombar', 'articulacao', 'dor nas costas'],
    label: 'Dor muscular / articular',
  ...
  {
    id: 'sono',
    keys: ['insonia', 'nao consigo dormir', 'ansiedade', 'agitacao', 'sono'],
    label: 'Insônia / ansiedade leve',
  ...
  {
    id: 'diarreia',
    keys: ['diarreia', 'desinteria', 'intestino solto', 'evacuacao'],
    ...
    options: [
      ...
      { name: 'Probióticos (Saccharomyces boulardii)', contra: [], caution: [], note: 'Podem reduzir a duração do quadro.' },
    ],
    ...
    sources: ['Caderno de Atenção Básica — Diarreia (MS)', 'OMS — terapia de reidratação oral', 'Bula — probióticos (ANVISA)'],
  },
];
...
export function analyze(question: string, p: Patient): Consultation {
  ...
  if (/hepat|figado|cirrose/.test(condText)) {
    tags.add('hepatico');
    tags.add('hepatopatia');
  }
  if (/ulcera|gastrite/.test(condText)) tags.add('ulcera');
  if (/asma|bronqu/.test(condText)) {
    tags.add('asma');
    tags.add('asma-sensivel');
  }
  ...
  if (/(anticoagul|varfarina|xarelto|rivaroxabana|marevan|aas diario|aspirina diaria)/.test(medsText)) {
  ...
  const assessments: OptionAssessment[] = (entry?.options ?? []).map((o) => {
    ...
    if (status !== 'contra') {
      ...
      if (tags.has('crianca')) {
        status = 'caution';
        reasons.push(TAG_LABEL.crianca);
      }
    }
