...
type AppView = 'welcome' | 'record' | 'vitals' | 'diary' | 'profiles' | 'insurance' | 'consultant';
...
export default function V1PreviewApp() {
  ...
  const menu = user && menuOpen ? <div className="mb-5 rounded-2xl border border-line bg-white p-2 shadow-lift"><div className="grid min-w-0 gap-1 sm:grid-cols-2 lg:grid-cols-3">
    {([['welcome', 'Início'], ['record', 'Prontuário'], ['vitals', 'Sinais vitais'], ['diary', 'Meu Diário'], ['insurance', 'Convênios'], ['consultant', 'Consultor'], ['profiles', 'Perfis e dependentes']] as [AppView, string][]).map(([key, label]) => <button key={key} onClick={() => go(key)} className={`rounded-xl px-4 py-3 text-left text-sm font-bold ${view === key ? 'bg-moss-50 text-moss-800' : 'text-ink hover:bg-paper'}`}>{label}</button>)}
  ...
  const activeView = view === 'welcome' ? welcomeView : view === 'profiles' ? profilesView : view === 'vitals' ? vitalsView : view === 'diary' ? diaryView : view === 'insurance' ? insuranceView : view === 'consultant' ? consultantView : recordView;
