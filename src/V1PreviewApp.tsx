...
export default function V1PreviewApp() {
  ...
  const createEvent = () => run(async () => {
    ...
    const input = {
      ...
      payload: { symptoms: symptoms.trim(), diagnosis: diagnosis.trim(), exams: examsText.trim(), prescriptions: prescriptions.trim(), notes: notes.trim() },
    };
  ...
  const startEditEvent = (event: HealthEventV1) => {
    ...
    setProfession(event.professionSnapshot ?? '');
  ...
  const recordList = <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-mute">Prontuário ativo</p><h2 className="font-display text-2xl font-bold text-ink">{activeProfile?.name ?? 'Escolha um perfil'}</h2></div>{activeProfile && <div className="flex flex-wrap gap-2"><SecondaryButton onClick={() => setShowInactiveRecords((v) => !v)}>{showInactiveRecords ? 'Ver ativos' : 'Ver atendimentos inativos'}</SecondaryButton><PrimaryButton onClick={() => { if (showRecordForm) { resetRecordForm(); setShowRecordForm(false); } else { resetRecordForm(); setShowRecordForm(true); } }}>{showRecordForm ? 'Cancelar' : '+ Adicionar Atendimento'}</PrimaryButton></div>}</div>{!activeProfile ? <p className="mt-4 text-sm text-mute">Escolha um perfil em Perfis e dependentes.</p> : clinicalEvents.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-line bg-white p-5 text-sm text-mute">Nenhum atendimento clínico ainda. Use “+ Adicionar registro” quando quiser incluir o primeiro.</div> : <div className="mt-5 overflow-hidden rounded-xl border border-line bg-white"><div className="hidden grid-cols-[180px_1fr_1.2fr_1.4fr_90px] gap-3 border-b border-line bg-paper px-4 py-3 text-xs font-bold uppercase tracking-wide text-mute md:grid"><span>Data/Hora</span><span>Tipo</span><span>Especialidade</span><span>Médico/Atendente</span><span></span></div><div className="divide-y divide-line">{clinicalEvents.map((event) => <details key={event.id} className="group min-w-0"><summary className="cursor-pointer list-none px-4 py-4 hover:bg-paper"><div className="grid min-w-0 grid-cols-2 gap-x-3 gap-y-3 md:grid-cols-[180px_1fr_1.2fr_1.4fr_90px] md:items-center"><div className="min-w-0"><span className="block text-[10px] font-bold uppercase text-mute md:hidden">Data/Hora</span><time className="break-words text-sm font-semibold text-ink">{new Date(event.occurredAt).toLocaleString('pt-BR')}</time></div><div className="min-w-0"><span className="block text-[10px] font-bold uppercase text-mute md:hidden">Tipo</span><span className="break-words text-sm text-ink">{EVENT_TYPES.find(([value]) => value === event.type)?.[1] ?? event.type}</span></div><div className="min-w-0"><span className="block text-[10px] font-bold uppercase text-mute md:hidden">Especialidade</span><span className="break-words text-sm text-ink">{event.professionSnapshot || '—'}</span></div><div className="min-w-0"><span className="block text-[10px] font-bold uppercase text-mute md:hidden">Médico/Atendente</span><span className="break-words text-sm text-ink">{event.practitionerNameSnapshot || '—'}</span></div><span className="col-span-2 text-xs font-bold text-moss-700 group-open:hidden md:col-span-1 md:text-right">Ver detalhes</span><span className="col-span-2 hidden text-xs font-bold text-moss-700 group-open:block md:col-span-1 md:text-right">Fechar</span></div></summary><div className="border-t border-line bg-moss-50/40 px-4 py-4"><div className="grid min-w-0 gap-3 text-sm sm:grid-cols-2"><div><strong className="block text-xs uppercase text-mute">Atendimento(descrição)</strong><p className="mt-1 break-words text-ink">{event.title}</p></div>{event.organizationNameSnapshot && <div><strong className="block text-xs uppercase text-mute">Hospital/Clínica/Consultório</strong><p className="mt-1 break-words text-ink">{event.organizationNameSnapshot}</p></div>}<div><strong className="block text-xs uppercase text-mute">Tipo</strong><p className="mt-1 text-ink">{EVENT_TYPES.find(([value]) => value === event.type)?.[1] ?? event.type}</p></div>{event.practitionerNameSnapshot && <div><strong className="block text-xs uppercase text-mute">Médico/Fisioterapeuta/Atendente</strong><p className="mt-1 break-words text-ink">{event.practitionerNameSnapshot}</p></div>}{event.professionSnapshot && <div><strong className="block text-xs uppercase text-mute">Especialidade</strong><p className="mt-1 break-words text-ink">{event.professionSnapshot}</p></div>}{event.registrationSnapshot && <div><strong className="block text-xs uppercase text-mute">Registro profissional</strong><p className="mt-1 text-ink">{event.councilSnapshot ?? ''} {event.registrationSnapshot}{event.registrationRegionSnapshot ? `/${event.registrationRegionSnapshot}` : ''}</p></div>}{typeof event.payload?.symptoms === 'string' && event.payload.symptoms && <div className="sm:col-span-2"><strong className="block text-xs uppercase text-mute">Sintomas / Queixa principal</strong><p className="mt-1 whitespace-pre-wrap break-words text-ink">{event.payload.symptoms}</p></div>}{typeof event.payload?.diagnosis === 'string' && event.payload.diagnosis && <div className="sm:col-span-2"><strong className="block text-xs uppercase text-mute">Diagnóstico / Causa / Hipótese</strong><p className="mt-1 whitespace-pre-wrap break-words text-ink">{event.payload.diagnosis}</p></div>}{typeof event.payload?.exams === 'string' && event.payload.exams && <div className="sm:col-span-2"><strong className="block text-xs uppercase text-mute">Exames</strong><p className="mt-1 whitespace-pre-wrap break-words text-ink">{event.payload.exams}</p></div>}{typeof event.payload?.prescriptions === 'string' && event.payload.prescriptions && <div className="sm:col-span-2"><strong className="block text-xs uppercase text-mute">Receitas / Prescrições</strong><p className="mt-1 whitespace-pre-wrap break-words text-ink">{event.payload.prescriptions}</p></div>}{typeof event.payload?.notes === 'string' && event.payload.notes && <div className="sm:col-span-2"><strong className="block text-xs uppercase text-mute">Observações</strong><p className="mt-1 whitespace-pre-wrap break-words text-ink">{event.payload.notes}</p></div>}<div className="sm:col-span-2 flex flex-wrap gap-2 border-t border-line pt-3">{event.status !== 'cancelled' ? <><SecondaryButton onClick={() => startEditEvent(event)}>Editar</SecondaryButton><button type="button" onClick={() => inactivateEvent(event)} className="rounded-xl border border-danger-500 px-4 py-3 text-sm font-bold text-danger-600">Inativar</button></> : <PrimaryButton onClick={() => reactivateEvent(event)}>Reativar</PrimaryButton>}</div></div></div></details>)}</div></div>}</Card>;

  const recordView = <div className="space-y-5">{showRecordForm && activeProfile && <Card><h3 className="font-display text-xl font-bold text-ink">{editingEventId ? 'Editar registro' : 'Novo Atendimento'}</h3><div className="mt-4 grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2"><label className="min-w-0 text-xs font-bold text-mute">Data/Hora<input type="datetime-local" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className={`${inputClass()} mt-1`} /></label><label className="min-w-0 text-xs font-bold text-mute">Hospital/Clínica/Consultório<input value={organizationName} onChange={(e) => setOrganizationName(e.target.value)} className={`${inputClass()} mt-1`} /></label><label className="min-w-0 text-xs font-bold text-mute">Tipo<select value={eventType} onChange={(e) => setEventType(e.target.value)} className={`${inputClass()} mt-1`}>{EVENT_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="min-w-0 text-xs font-bold text-mute">CRM/CREFITO<div className="mt-1 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_110px]"><select value={council} onChange={(e) => setCouncil(e.target.value)} className={inputClass()}><option value="CRM">CRM</option><option value="CREFITO">Crefito</option><option value="Outros">Outros</option></select><input value={registration} onChange={(e) => setRegistration(e.target.value)} placeholder="Número" className={inputClass()} /><select value={registrationRegion} onChange={(e) => setRegistrationRegion(e.target.value)} className={inputClass()}><option value="">UF</option>{BRAZIL_UFS.map((uf) => <option key={uf} value={uf}>{uf}</option>)}</select></div></label><label className="min-w-0 text-xs font-bold text-mute">Nome do Médico/Fisioterapeuta/Atendente<input value={practitionerName} onChange={(e) => setPractitionerName(e.target.value)} className={`${inputClass()} mt-1`} /></label><label className="min-w-0 text-xs font-bold text-mute">Especialidade<input value={profession} onChange={(e) => setProfession(e.target.value)} className={`${inputClass()} mt-1`} /></label><label className="min-w-0 text-xs font-bold text-mute md:col-span-2">Atendimento(descrição)<input value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} className={`${inputClass()} mt-1`} /></label><label className="min-w-0 text-xs font-bold text-mute md:col-span-2">Sintomas / Queixa principal<textarea value={symptoms} onChange={(e) => setSymptoms(e.target.value)} className={`${inputClass()} mt-1 min-h-20`} /></label><label className="min-w-0 text-xs font-bold text-mute md:col-span-2">Diagnóstico / Causa / Hipótese<textarea value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} className={`${inputClass()} mt-1 min-h-20`} /></label><label className="min-w-0 text-xs font-bold text-mute md:col-span-2">Exames<textarea value={examsText} onChange={(e) => setExamsText(e.target.value)} className={`${inputClass()} mt-1 min-h-20`} /></label><label className="min-w-0 text-xs font-bold text-mute md:col-span-2">Receitas / Prescrições<textarea value={prescriptions} onChange={(e) => setPrescriptions(e.target.value)} className={`${inputClass()} mt-1 min-h-20`} /></label><label className="min-w-0 text-xs font-bold text-mute md:col-span-2">Observações<textarea value={notes} onChange={(e) => setNotes(e.target.value)} className={`${inputClass()} mt-1 min-h-24`} /></label><div className="md:col-span-2 rounded-xl border border-line bg-paper p-4"><p className="text-xs font-bold uppercase tracking-wide text-mute">Anexos do Atendimento</p><p className="mt-1 text-xs text-mute">Você pode anexar vários PDFs ou imagens em cada categoria. Arquivos já salvos permanecem vinculados ao Atendimento.</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-line bg-white p-4 shadow-sm"><p className="text-sm font-bold text-ink">📄 Laudo / Relatório</p><label className="mt-2 inline-flex cursor-pointer rounded-lg border border-moss-500 px-3 py-2 text-xs font-bold text-moss-800">+ Selecionar arquivos<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" multiple className="hidden" onChange={(e) => setReportFiles((current) => [...current, ...Array.from(e.target.files ?? [])])} /></label>{reportFiles.length > 0 && <div className="mt-2 space-y-1">{reportFiles.map((file, index) => <div key={`${file.name}-${index}`} className="flex items-center justify-between gap-2 text-xs"><span className="truncate">{file.name}</span><button type="button" className="font-bold text-danger-600" onClick={() => setReportFiles((current) => current.filter((_, i) => i !== index))}>Remover</button></div>)}</div>}{savedDocuments.filter((doc) => doc.type === 'report').map((doc) => <div key={doc.id} className="mt-2 rounded-lg bg-paper px-2 py-2 text-xs"><div className="flex min-w-0 items-center justify-between gap-2"><span className="min-w-0 flex-1 truncate font-semibold text-ink" title={doc.originalFilename}>{doc.originalFilename}</span><span className="shrink-0 text-moss-700">Salvo</span></div><button type="button" className="mt-1 text-xs font-bold text-moss-800 underline" onClick={() => activeProfile && editingEventId && void api.openHealthEventDocument(activeProfile.id, editingEventId, doc.id, doc.originalFilename)}>Visualizar</button></div>)}</div><div className="rounded-2xl border border-line bg-white p-4 shadow-sm"><p className="text-sm font-bold text-ink">💊 Receita / Prescrição</p><label className="mt-2 inline-flex cursor-pointer rounded-lg border border-moss-500 px-3 py-2 text-xs font-bold text-moss-800">+ Selecionar arquivos<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" multiple className="hidden" onChange={(e) => setPrescriptionFiles((current) => [...current, ...Array.from(e.target.files ?? [])])} /></label>{prescriptionFiles.length > 0 && <div className="mt-2 space-y-1">{prescriptionFiles.map((file, index) => <div key={`${file.name}-${index}`} className="flex items-center justify-between gap-2 text-xs"><span className="truncate">{file.name}</span><button type="button" className="font-bold text-danger-600" onClick={() => setPrescriptionFiles((current) => current.filter((_, i) => i !== index))}>Remover</button></div>)}</div>}{savedDocuments.filter((doc) => doc.type === 'prescription').map((doc) => <div key={doc.id} className="mt-2 rounded-lg bg-paper px-2 py-2 text-xs"><div className="flex min-w-0 items-center justify-between gap-2"><span className="min-w-0 flex-1 truncate font-semibold text-ink" title={doc.originalFilename}>{doc.originalFilename}</span><span className="shrink-0 text-moss-700">Salvo</span></div><button type="button" className="mt-1 text-xs font-bold text-moss-800 underline" onClick={() => activeProfile && editingEventId && void api.openHealthEventDocument(activeProfile.id, editingEventId, doc.id, doc.originalFilename)}>Visualizar</button></div>)}</div><div className="rounded-2xl border border-line bg-white p-4 shadow-sm"><p className="text-sm font-bold text-ink">🧪 Exames</p><label className="mt-2 inline-flex cursor-pointer rounded-lg border border-moss-500 px-3 py-2 text-xs font-bold text-moss-800">+ Selecionar arquivos<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" multiple className="hidden" onChange={(e) => setExamFiles((current) => [...current, ...Array.from(e.target.files ?? [])])} /></label>{examFiles.length > 0 && <div className="mt-2 space-y-1">{examFiles.map((file, index) => <div key={`${file.name}-${index}`} className="flex items-center justify-between gap-2 text-xs"><span className="truncate">{file.name}</span><button type="button" className="font-bold text-danger-600" onClick={() => setExamFiles((current) => current.filter((_, i) => i !== index))}>Remover</button></div>)}</div>}{savedDocuments.filter((doc) => doc.type === 'exam').map((doc) => <div key={doc.id} className="mt-2 rounded-lg bg-paper px-2 py-2 text-xs"><div className="flex min-w-0 items-center justify-between gap-2"><span className="min-w-0 flex-1 truncate font-semibold text-ink" title={doc.originalFilename}>{doc.originalFilename}</span><span className="shrink-0 text-moss-700">Salvo</span></div><button type="button" className="mt-1 text-xs font-bold text-moss-800 underline" onClick={() => activeProfile && editingEventId && void api.openHealthEventDocument(activeProfile.id, editingEventId, doc.id, doc.originalFilename)}>Visualizar</button></div>)}</div></div></div><div className="md:col-span-2"><PrimaryButton disabled={busy} onClick={() => void createEvent()}>{busy ? 'Salvando...' : editingEventId ? 'Salvar alterações' : 'Salvar no prontuário'}</PrimaryButton></div></div></Card>}{!editingEventId && recordList}</div>;
