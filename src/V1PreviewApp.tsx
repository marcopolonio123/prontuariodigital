  ...
  const createEvent = () => run(async () => {
    ...
    const created = await api.createHealthEvent(activeProfile.id, {
      ...
      payload: {
        ...
        exams: examsText.trim(),
        prescriptions: prescriptions.trim(),
        notes: notes.trim(),
      },
    });
    setEvents((current) => [created, ...current.filter((item) => item.id !== created.id)]);
    setEventTitle(''); setSymptoms(''); setDiagnosis(''); setExamsText(''); setPrescriptions(''); setNotes('');
    setShowRecordForm(false); setMessage('Registro salvo com sucesso.');
  });
