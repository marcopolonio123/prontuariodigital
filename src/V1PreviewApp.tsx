...
export default function V1PreviewApp() {
  ...
  const createInsurance = () => run(async () => {
    ...
    await loadEvents(activeProfile, api); setMessage('Convênio salvo no prontuário.');
  });
  ...
  const menu = user && menuOpen ? <div className="mb-5 rounded-2xl border border-line bg-white p-2 shadow-lift">
    <div className="grid gap-1 sm:grid-cols-2 lg:grid-cols-3">
      {([
        ['welcome', 'Início'], ['record', 'Prontuário'], ['vitals', 'Sinais vitais'], ['insurance', 'Convênios'], ['consultant', 'Consultor'], ['profiles', 'Perfis e dependentes'],
      ] as [AppView, string][]).map(([key, label]) => <button key={key} onClick={() => go(key)} className={`rounded-xl px-4 py-3 text-left text-sm font-bold ${view === key ? 'bg-moss-50 text-moss-800' : 'text-ink hover:bg-paper'}`}>{label}</button>)}
  ...
  const welcomeView = <div className="space-y-5">
    <Card><p className="text-xs font-bold uppercase tracking-wide text-moss-700">Bem-vindo ao MyDoctor</p><h2 className="mt-2 font-display text-3xl font-bold text-ink">Olá, {user?.name?.split(' ')[0]}.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-mute">O MyDoctor organiza seu histórico de saúde em um único prontuário. Você pode registrar consultas e exames, acompanhar sinais vitais, administrar dependentes, guardar seus convênios e usar o Consultor para preparar melhor sua próxima consulta.</p><div className="mt-5 flex flex-wrap gap-2"><button onClick={() => go('record')} className="rounded-xl bg-pine-900 px-4 py-3 text-sm font-bold text-white">Abrir meu prontuário</button><button onClick={() => go('vitals')} className="rounded-xl border border-moss-500 px-4 py-3 text-sm font-bold text-moss-800">Registrar sinais vitais</button></div></Card>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Card><h3 className="font-display text-lg font-bold text-ink">Prontuário longitudinal</h3><p className="mt-2 text-sm text-mute">Consultas, exames, procedimentos, receitas e outros eventos em ordem cronológica.</p></Card><Card><h3 className="font-display text-lg font-bold text-ink">Sinais vitais</h3><p className="mt-2 text-sm text-mute">Registro manual hoje e área preparada para Apple Health, Health Connect e dispositivos compatíveis.</p></Card><Card><h3 className="font-display text-lg font-bold text-ink">Privacidade</h3><p className="mt-2 text-sm text-mute">Cada registro fica associado ao perfil selecionado e à conta autenticada.</p></Card></div>
  </div>;

  const profilesView = <div className="grid gap-5 lg:grid-cols-2"><Card><p className="text-xs font-bold uppercase tracking-wide text-mute">Conta</p><h2 className="mt-1 font-display text-2xl font-bold text-ink">{user?.name}</h2><p className="text-sm text-mute">{user?.email}</p><h3 className="mt-6 font-display text-lg font-bold text-ink">Escolher prontuário</h3><div className="mt-3 space-y-2">{profiles.map((profile) => <button key={profile.id} onClick={() => void chooseProfile(profile)} className={`w-full rounded-xl border p-3 text-left ${activeProfile?.id === profile.id ? 'border-moss-500 bg-moss-50' : 'border-line bg-white'}`}><strong className="block text-sm text-ink">{profile.name}</strong><span className="text-xs text-mute">{profile.relationship} · {profile.source === 'owned' ? 'sob sua gestão' : 'delegado'}</span></button>)}</div></Card><Card><h2 className="font-display text-xl font-bold text-ink">Cadastrar dependente</h2><p className="mt-1 text-sm text-mute">Crie um prontuário para filho, pai/mãe ou pessoa sob sua responsabilidade.</p><input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nome completo" className={`${inputClass()} mt-4`} /><select value={relationship} onChange={(e) => setRelationship(e.target.value as typeof relationship)} className={`${inputClass()} mt-2`}><option value="child">Filho(a)</option><option value="parent">Pai/Mãe</option><option value="guardian">Pessoa sob tutela</option><option value="dependent">Dependente</option><option value="other">Outro</option></select><button disabled={busy} onClick={() => void createDependent()} className="mt-3 w-full rounded-xl border border-moss-500 px-3 py-3 text-sm font-bold text-moss-800">Adicionar perfil</button></Card></div>;

  const vitalsView = <div className="space-y-5"><Card><p className="text-xs font-bold uppercase tracking-wide text-moss-700">Sinais vitais</p><h2 className="mt-1 font-display text-2xl font-bold text-ink">{activeProfile?.name ?? 'Escolha um perfil'}</h2><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-moss-200 bg-moss-50 p-4"><strong className="text-sm text-ink">1. Manual</strong><p className="mt-1 text-xs text-mute">Já funciona: digite a medição abaixo e salve.</p></div><div className="rounded-xl border border-line bg-white p-4"><strong className="text-sm text-ink">2. Apple Health / HealthKit</strong><p className="mt-1 text-xs text-mute">A integração automática exige o MyDoctor instalado no iPhone; não é liberada pelo navegador.</p></div><div className="rounded-xl border border-line bg-white p-4"><strong className="text-sm text-ink">3. Android Health Connect</strong><p className="mt-1 text-xs text-mute">Também exige o app Android instalado e permissão do usuário.</p></div></div><p className="mt-4 rounded-xl bg-paper p-3 text-xs leading-5 text-mute"><strong>Importante:</strong> o celular sozinho não mede clinicamente pressão arterial, glicemia ou SpO₂. Esses dados podem vir de um aparelho/relógio compatível ou ser digitados manualmente.</p>{activeProfile && <div className="mt-5 grid gap-3 sm:grid-cols-2"><select value={vitalType} onChange={(e) => setVitalType(e.target.value as VitalType)} className={inputClass()}>{VITAL_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input type="datetime-local" value={vitalDate} onChange={(e) => setVitalDate(e.target.value)} className={inputClass()} /><input value={vitalValue} onChange={(e) => setVitalValue(e.target.value.replace(',', '.'))} inputMode="decimal" placeholder={vitalType === 'blood_pressure' ? 'Sistólica (ex.: 120)' : `Valor em ${selectedVital[2]}`} className={inputClass()} />{vitalType === 'blood_pressure' && <input value={vitalSecondaryValue} onChange={(e) => setVitalSecondaryValue(e.target.value.replace(',', '.'))} inputMode="decimal" placeholder="Diastólica (ex.: 80)" className={inputClass()} />}<select value={vitalSource} onChange={(e) => setVitalSource(e.target.value)} className={inputClass()}><option value="manual">Digitado manualmente</option><option value="healthkit">Apple Health / HealthKit</option><option value="health_connect">Android Health Connect</option><option value="bluetooth">Dispositivo Bluetooth</option><option value="institution">Instituição de saúde</option></select><input value={vitalDevice} onChange={(e) => setVitalDevice(e.target.value)} placeholder="Aparelho/dispositivo (opcional)" className={inputClass()} /><button disabled={busy} onClick={() => void createVital()} className="sm:col-span-2 rounded-xl bg-pine-900 px-4 py-3 font-bold text-white disabled:opacity-50">Salvar sinal vital</button></div>}</Card><Card><div className="flex items-center justify-between gap-3"><h2 className="font-display text-xl font-bold text-ink">Histórico de medições</h2>{activeProfile && <button disabled={busy} onClick={() => void loadEvents(activeProfile)} className="rounded-lg border border-line px-3 py-2 text-xs font-bold">Atualizar</button>}</div>{!activeProfile ? <p className="mt-3 text-sm text-mute">Escolha um perfil.</p> : vitalEvents.length === 0 ? <p className="mt-3 text-sm text-mute">Nenhum sinal vital registrado.</p> : <div className="mt-4 grid gap-3 sm:grid-cols-2">{vitalEvents.map((event) => { const payload = vitalPayload(event); return <article key={event.id} className="rounded-xl border border-line bg-white p-4"><p className="text-xs font-bold uppercase tracking-wide text-moss-700">{String(payload.label ?? 'Sinal vital')}</p><h3 className="mt-1 text-xl font-bold text-ink">{String(payload.value ?? '')}{payload.secondaryValue ? `/${String(payload.secondaryValue)}` : ''} <span className="text-sm font-semibold text-mute">{String(payload.unit ?? '')}</span></h3><p className="mt-1 text-xs text-mute">Origem: {String(payload.source ?? 'manual')}{payload.device ? ` · ${String(payload.device)}` : ''}</p><time className="mt-2 block text-xs text-mute">{new Date(event.occurredAt).toLocaleString('pt-BR')}</time></article>; })}</div>}</Card></div>;

  const insuranceView = <div className="grid gap-5 lg:grid-cols-2"><Card><p className="text-xs font-bold uppercase tracking-wide text-moss-700">Convênios</p><h2 className="mt-1 font-display text-2xl font-bold text-ink">{activeProfile?.name ?? 'Escolha um perfil'}</h2><p className="mt-2 text-sm text-mute">Cadastre plano de saúde, número da carteirinha e validade. O registro fica salvo no prontuário do perfil selecionado.</p>{activeProfile && <div className="mt-5 space-y-3"><input value={insuranceProvider} onChange={(e) => setInsuranceProvider(e.target.value)} placeholder="Convênio / operadora *" className={inputClass()} /><input value={insurancePlan} onChange={(e) => setInsurancePlan(e.target.value)} placeholder="Plano / categoria" className={inputClass()} /><input value={insuranceNumber} onChange={(e) => setInsuranceNumber(e.target.value)} placeholder="Número da carteirinha" className={inputClass()} /><input value={insuranceHolder} onChange={(e) => setInsuranceHolder(e.target.value)} placeholder="Nome do titular" className={inputClass()} /><label className="block text-xs font-bold text-mute">Validade da carteirinha<input type="date" value={insuranceValidity} onChange={(e) => setInsuranceValidity(e.target.value)} className={`${inputClass()} mt-1`} /></label><textarea value={insuranceNotes} onChange={(e) => setInsuranceNotes(e.target.value)} placeholder="Observações (opcional)" className={`${inputClass()} min-h-20`} /><button disabled={busy} onClick={() => void createInsurance()} className="w-full rounded-xl bg-pine-900 px-4 py-3 font-bold text-white disabled:opacity-50">Salvar convênio</button></div>}</Card><Card><h2 className="font-display text-xl font-bold text-ink">Convênios cadastrados</h2>{insuranceEvents.length === 0 ? <p className="mt-3 text-sm text-mute">Nenhum convênio cadastrado para este perfil.</p> : <div className="mt-4 space-y-3">{insuranceEvents.map((event) => { const payload = event.payload as InsurancePayload; return <article key={event.id} className="rounded-xl border border-line bg-white p-4"><h3 className="font-display text-lg font-bold text-ink">{payload.provider || event.title}</h3>{payload.planName && <p className="text-sm text-mute">{payload.planName}</p>}<div className="mt-3 space-y-1 text-sm text-ink">{payload.memberNumber && <p><strong>Carteirinha:</strong> {payload.memberNumber}</p>}{payload.holderName && <p><strong>Titular:</strong> {payload.holderName}</p>}{payload.validity && <p><strong>Validade:</strong> {new Date(`${payload.validity}T12:00:00`).toLocaleDateString('pt-BR')}</p>}{payload.notes && <p><strong>Observações:</strong> {payload.notes}</p>}</div></article>; })}</div>}</Card></div>;

  const consultantSummary = (() => {
    const latestVitals = new Map<string, HealthEventV1>();
    vitalEvents.forEach((event) => { const type = String(event.payload?.vitalType ?? 'vital'); if (!latestVitals.has(type)) latestVitals.set(type, event); });
    return { latestVitals: [...latestVitals.values()], recentClinical: clinicalEvents.slice(0, 5), insurance: insuranceEvents[0] };
  })();

  const consultantView = <div className="space-y-5"><Card><p className="text-xs font-bold uppercase tracking-wide text-moss-700">Consultor MyDoctor</p><h2 className="mt-1 font-display text-2xl font-bold text-ink">Prepare sua próxima consulta</h2><p className="mt-2 text-sm leading-6 text-mute">Esta primeira versão do Consultor organiza o que já existe no seu prontuário para facilitar a conversa com o profissional de saúde. Ele não faz diagnóstico e não substitui orientação médica.</p></Card><div className="grid gap-5 lg:grid-cols-2"><Card><h3 className="font-display text-xl font-bold text-ink">Resumo de {activeProfile?.name ?? 'perfil'}</h3>{!activeProfile ? <p className="mt-3 text-sm text-mute">Escolha um perfil primeiro.</p> : <div className="mt-4 space-y-4"><div><p className="text-xs font-bold uppercase tracking-wide text-mute">Últimos sinais vitais</p>{consultantSummary.latestVitals.length === 0 ? <p className="mt-1 text-sm text-mute">Nenhum sinal vital registrado.</p> : consultantSummary.latestVitals.map((event) => <p key={event.id} className="mt-1 text-sm text-ink">• {event.title} <span className="text-mute">({new Date(event.occurredAt).toLocaleDateString('pt-BR')})</span></p>)}</div><div><p className="text-xs font-bold uppercase tracking-wide text-mute">Convênio</p><p className="mt-1 text-sm text-ink">{consultantSummary.insurance?.title ?? 'Nenhum convênio cadastrado.'}</p></div><div><p className="text-xs font-bold uppercase tracking-wide text-mute">Eventos recentes</p>{consultantSummary.recentClinical.length === 0 ? <p className="mt-1 text-sm text-mute">Nenhum evento clínico registrado.</p> : consultantSummary.recentClinical.map((event) => <p key={event.id} className="mt-1 text-sm text-ink">• {event.title} <span className="text-mute">({new Date(event.occurredAt).toLocaleDateString('pt-BR')})</span></p>)}</div></div>}</Card><Card><h3 className="font-display text-xl font-bold text-ink">Perguntas para levar à consulta</h3><div className="mt-4 space-y-3 text-sm text-ink"><p className="rounded-xl bg-paper p-3">Há alguma mudança importante nos meus sinais vitais recentes?</p><p className="rounded-xl bg-paper p-3">Preciso atualizar exames, vacinas ou medicações?</p><p className="rounded-xl bg-paper p-3">Quais sintomas ou alterações devo acompanhar até a próxima consulta?</p><p className="rounded-xl bg-paper p-3">Existe alguma orientação específica considerando meu histórico registrado?</p></div></Card></div></div>;

  const recordView = <div className="space-y-5"><Card><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-mute">Prontuário ativo</p><h2 className="font-display text-2xl font-bold text-ink">{activeProfile?.name ?? 'Escolha um perfil'}</h2></div>{activeProfile && <button disabled={busy} onClick={() => void loadEvents(activeProfile)} className="rounded-lg border border-line px-3 py-2 text-xs font-bold">Atualizar linha do tempo</button>}</div>{activeProfile && <div className="mt-5 grid gap-3 md:grid-cols-2"><select value={eventType} onChange={(e) => setEventType(e.target.value)} className={inputClass()}>{EVENT_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input type="datetime-local" value={eventDate} onChange={(e) => setEventDate(e.target.value)} className={inputClass()} /><input value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} placeholder="Descrição do registro *" className={`${inputClass()} md:col-span-2`} /><input value={organizationName} onChange={(e) => setOrganizationName(e.target.value)} placeholder="Hospital / clínica / consultório" className={inputClass()} /><input value={locationName} onChange={(e) => setLocationName(e.target.value)} placeholder="Local / unidade" className={inputClass()} /><input value={practitionerName} onChange={(e) => setPractitionerName(e.target.value)} placeholder="Profissional de saúde" className={inputClass()} /><input value={profession} onChange={(e) => setProfession(e.target.value)} placeholder="Profissão / especialidade" className={inputClass()} /><input value={council} onChange={(e) => setCouncil(e.target.value)} placeholder="Conselho (CRM, CREFITO...)" className={inputClass()} /><div className="grid grid-cols-[1fr_84px] gap-2"><input value={registration} onChange={(e) => setRegistration(e.target.value)} placeholder="Registro" className={inputClass()} /><input value={registrationRegion} onChange={(e) => setRegistrationRegion(e.target.value)} placeholder="UF" className={inputClass()} /></div><textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Observações clínicas" className={`${inputClass()} min-h-24 md:col-span-2`} /><button disabled={busy} onClick={() => void createEvent()} className="md:col-span-2 rounded-xl bg-pine-900 px-4 py-3 font-bold text-white disabled:opacity-50">Salvar no prontuário</button></div>}</Card><Card><h2 className="font-display text-xl font-bold text-ink">Linha do tempo clínica</h2>{!activeProfile ? <p className="mt-3 text-sm text-mute">Escolha um perfil.</p> : clinicalEvents.length === 0 ? <p className="mt-3 text-sm text-mute">Nenhum evento clínico registrado.</p> : <ol className="mt-4 space-y-3">{clinicalEvents.map((event) => <li key={event.id} className="rounded-xl border border-line bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><span className="text-[10px] font-bold uppercase tracking-wider text-moss-700">{event.type}</span><h3 className="font-display text-lg font-bold text-ink">{event.title}</h3></div><time className="font-mono text-xs text-mute">{new Date(event.occurredAt).toLocaleString('pt-BR')}</time></div><div className="mt-2 text-sm text-mute">{event.organizationNameSnapshot && <p><strong>Instituição:</strong> {event.organizationNameSnapshot}</p>}{event.locationNameSnapshot && <p><strong>Local:</strong> {event.locationNameSnapshot}</p>}{event.practitionerNameSnapshot && <p><strong>Profissional:</strong> {event.practitionerNameSnapshot} {event.professionSnapshot ? `· ${event.professionSnapshot}` : ''}</p>}{event.registrationSnapshot && <p><strong>Registro:</strong> {event.councilSnapshot ?? ''} {event.registrationSnapshot}{event.registrationRegionSnapshot ? `/${event.registrationRegionSnapshot}` : ''}</p>}{typeof event.payload?.notes === 'string' && event.payload.notes && <p className="mt-2 whitespace-pre-wrap text-ink">{event.payload.notes}</p>}</div></li>)}</ol>}</Card></div>;
  ...
  return <div className="min-h-screen bg-paper"><div className="mx-auto max-w-6xl p-4 pb-[calc(2rem+env(safe-area-inset-bottom))] md:p-8">
    <header className="mb-4 flex items-start justify-between gap-3"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-moss-700">MyDoctor</p><h1 className="font-display text-2xl font-bold text-ink sm:text-3xl">Prontuário longitudinal</h1><p className="mt-1 text-sm text-mute">Seu histórico de saúde em um único lugar.</p></div>{user && <button type="button" onClick={() => setMenuOpen((value) => !value)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-line bg-white text-ink shadow-sm" aria-label={menuOpen ? 'Fechar menu' : 'Abrir menu'} title={menuOpen ? 'Fechar menu' : 'Abrir menu'}><MenuIcon open={menuOpen} /></button>}</header>
    {menu}
    ...
    {!user ? <div className="mx-auto max-w-md pt-4 sm:pt-10">{authPanel()}</div> : activeView}
  </div></div>;
}
