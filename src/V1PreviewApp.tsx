...
export default function V1PreviewApp() {
  ...
  return <div className="min-h-screen bg-paper pb-[calc(7rem+env(safe-area-inset-bottom))] md:p-8 md:pb-8"><div className="mx-auto max-w-6xl p-4 md:p-0">
    ...
    {!user ? <div className="mx-auto max-w-md pt-4 sm:pt-10">{authPanel()}</div> : <>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-line bg-white px-2 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] shadow-2xl md:hidden"><button onClick={() => setView('record')} className={`rounded-xl px-2 py-2 text-xs font-bold ${view === 'record' ? 'bg-pine-900 text-white' : 'text-ink'}`}>Prontuário</button><button onClick={() => setView('vitals')} className={`rounded-xl px-2 py-2 text-xs font-bold ${view === 'vitals' ? 'bg-pine-900 text-white' : 'text-ink'}`}>Sinais vitais</button><button onClick={() => setView('profiles')} className={`rounded-xl px-2 py-2 text-xs font-bold ${view === 'profiles' ? 'bg-pine-900 text-white' : 'text-ink'}`}>Perfis</button><button onClick={logout} className="rounded-xl px-2 py-2 text-xs font-bold text-danger-600">Sair</button></nav>
    </>}
  </div></div>;
}
